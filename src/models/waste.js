...
const getWasteById = (wasteId) => {
    const SQLQuery ="SELECT waste.name, waste.description, waste.characteristics, waste.impacts, waste.image, contents.name as content_name FROM waste left join contents on waste.contentId = contents.contentId WHERE waste.wasteId = ?";
...
const createNewWaste = (body, image) => {
    ...
    const contentValues = [body.name, body.description, body.characteristics, body.impacts, image.buffer, body.recyclingId, body.contentId];
...
const updateWaste = (body, wasteId, image) => {
    ...
    const contentValues = [body.name, body.description, body.characteristics, body.impacts, image.buffer, body.contentId, wasteId];
    return dbPool.execute(SQLQuery, contentValues);
}
